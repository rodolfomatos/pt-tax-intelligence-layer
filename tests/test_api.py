...
class TestSearchEndpoint:
    ...
    async def test_search_requires_query(self):
        ...
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/tax/search")
    ...
    async def test_search_with_query(self):
        ...
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/tax/search?q=iva&limit=5")
...
class TestArticleEndpoint:
    ...
    async def test_article_not_found(self):
        ...
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/tax/article/CIVA/999")
...
class TestValidateEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_validate_warns_on_low_sources_high_confidence(self):
...
class TestDecisionsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_decisions_returns_list_or_error(self):
        """Decisions should return a list or graceful error."""
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/tax/decisions")
