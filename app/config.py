...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="postgresql://postgres:postgres@localhost:5432/tax_intelligence",
        alias="DATABASE_URL",
    )
    ...
    @field_validator("use_iaedu", mode="before")
    @classmethod
    def _parse_bool(cls, v):
        """Parse USE_IAEDU from string, handling malformed env values."""
        if isinstance(v, bool):
            return v
        if isinstance(v, str):
            # Take first token if there's concatenation
