...
class GraphQuery:
    ...
    async def query_as_of(
        ...
        async with get_db_session() as session:
            query = select(GraphNode).where(
                ...
                or_(
                    GraphNode.valid_to == None,
                    GraphNode.valid_to > as_of
                )
            )
        ...
        async with get_db_session() as session:
            ...
            entity_result = await session.execute(
                select(GraphNode).where(
                    GraphNode.external_id == entity_external_id
                )
            )
    ...
    async def find_similar(
        ...
        async with get_db_session() as session:
            ...
            similar_result = await session.execute(
                select(GraphEdge, GraphNode).join(
    ...
    async def find_contradictions(
        ...
        async with get_db_session() as session:
            query = select(Contradiction).where(Contradiction.resolved == False)
        ...
        async with get_db_session() as session:
            ...
            edges_result = await session.execute(
                select(
                    ...
                    func.count(GraphEdge.id).label("count")
            ...
            contra_result = await session.execute(
                select(func.count(Contradiction.id)).where(Contradiction.resolved == False)
            )
